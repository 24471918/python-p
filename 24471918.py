...
def main(CSVfile, TXTfile, category):
    ...
    # Validate data and collect valid entries
    valid_data = []
    for row in all_data:
        try:
            ...
            print(f"Error processing row: {row}. Error: {e}. Skipping this row.")
            continue

    # Check if there are any valid data entries
    if not valid_data:
        print("No valid data found in the CSV file after validation.")
        return None, None, None, None  # Ensure we return 4 values
